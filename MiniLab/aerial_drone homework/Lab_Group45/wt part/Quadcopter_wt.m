...
segment_length = floor(size(t_ref,2) / 3);
wind_speed_x = repmat([10 0 0], segment_length, 1);
wind_speed_y = repmat([0 10 0], segment_length, 1);
wind_speed_z = repmat([0 0 10], segment_length, 1);

wind_speed_series = [wind_speed_x; wind_speed_y; wind_speed_z];

if size(wind_speed_series,1) < size(t_ref,2)
...
figure('Name','Wind Speed Segments');
plot(t_ref, wind_speed_series(:,1), 'r', 'DisplayName','Vw_x');
hold on;
plot(t_ref, wind_speed_series(:,2), 'g', 'DisplayName','Vw_y');
plot(t_ref, wind_speed_series(:,3), 'b', 'DisplayName','Vw_z');
xlabel('Time (s)');
ylabel('Wind Speed (m/s)');
title('Wind Speed Series Over Time');
legend;
grid on;


%Run the simulink simulation (Need to finish the TODOs in the simulink before running the simulation!)
out = sim('Quadcopter_simulation_wt',t);
